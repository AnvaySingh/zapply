# DEPLOY.md — hosting the apply-copilot demo web app
...
3. Push this repo's files to the Space (it's a git repo):
   ```bash
   git remote add space https://huggingface.co/spaces/<you>/apply-copilot
   git push space main
   ```
...
5. The Space builds the image (installs deps + bakes data/model — a few minutes) and serves at
   `https://<you>-apply-copilot.hf.space`.
...
```bash
az group create -n apply-copilot-rg -l centralindia
az containerapp up \
  --name apply-copilot --resource-group apply-copilot-rg \
...
az containerapp secret set -n apply-copilot -g apply-copilot-rg --secrets gemini=<key>
az containerapp update -n apply-copilot -g apply-copilot-rg \
...
`docker build -t apply-copilot . && docker run -p 7860:7860 -e PUBLIC_MODE=on -e ACCESS_CODE=code apply-copilot`
...
- **Vercel / Netlify can't *run* this app** (it's Python + PyTorch, not static/edge JS) — but they
  ...
  ```json
  { "rewrites": [ { "source": "/(.*)", "destination": "https://<you>-apply-copilot.hf.space/$1" } ] }
  ```
  Now `your-name.vercel.app` (or a custom domain you add to Vercel, free) serves your HF-hosted app.
  Netlify: a `_redirects` file with `/*  https://<you>-apply-copilot.hf.space/:splat  200`.
